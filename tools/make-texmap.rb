...
class TexMapCreator
    ...
    def load_alpha_image(name, width, height, premultiply_alpha=true, background_color='#000000')
        ...
        if premultiply_alpha && (img.rows > 128 || img.columns > 128)
            pixels = img
            alpha = pixels.channel(Magick::OpacityChannel).negate
            ...
            img = pixels
            img.composite!(alpha, 0, 0, Magick::CopyOpacityCompositeOp)
        end
...
def process(mode)
    if mode == 'builder'
        src_dir = '/var/www/zame-dev.org/projects/fsr-mapper/graphics'
        dst_base = '/var/www/zame-dev.org/projects/fsr-mapper/build/.srvbuild'
        suffix = '-v4'
    else
        src_dir = File.dirname(__FILE__) + '/graphics'
        dst_base = File.dirname(__FILE__) + '/../src/main'
        suffix = nil
    end
